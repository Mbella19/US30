...
def load_and_process_full_data(config: Config, training_baseline=None):
...
def run_simulation(args):
    ...
    if baseline_path.exists():
        try:
            ...
            logger.info(f"v37 TrainingBaseline loaded from {baseline_path}")
        except Exception as e:
            logger.warning(f"Failed to load training baseline: {e}")
    else:
        logger.warning(f"No training baseline found at {baseline_path}")
        logger.warning("v37 OOD features will use zero placeholders")

    # Load Data
    df_5m, df_15m, df_45m = load_and_process_full_data(config, training_baseline=training_baseline)
